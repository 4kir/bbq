module ApplicationHelper
  ...
  def bootstrap_flash(opts = {})
    flash.each do |msg_type, message|
      concat(content_tag(:div, message, class: "alert alert-warning alert-dismissible fade show", role: 'alert') do
        concat message
        concat content_tag(:button, '', class: 'btn-close', data: { dismiss: 'alert' })
      end)
    end
    nil
  end
